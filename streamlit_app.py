...
# 标题区域
st.markdown('<div class="main-header">🎯 微博情感分析研究平台</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">基于大语言模型的学年论文研究成果演示 + 动态研究数据</div>', unsafe_allow_html=True)
...
# 主界面标签页 - 结合两类功能
tab1, tab2, tab3, tab4 = st.tabs(["🔍 情感分析", "📚 研究案例", "📊 动态研究", "🎓 关于研究"])
...
with tab1:
    ...
    # 真实标签选择 - 新增研究数据记录功能
    true_sentiment = st.selectbox("选择真实情感标签（用于研究数据积累）:", 
                                 ["积极", "消极", "中性", "不记录"], 
                                 key="true_label")
    ...
    if st.button("🚀 开始情感分析", type="primary", use_container_width=True):
        if user_input.strip():
            with st.spinner("AI正在分析情感..."):
                ...
                if use_api:
                    sentiment, confidence, status, analysis_time = analyze_sentiment_api(user_input)
                else:
                    sentiment, confidence, status, analysis_time = analyze_sentiment_local(user_input)
            ...
            # 记录研究数据 - 新增功能
            if true_sentiment != "不记录":
                st.session_state.research_data['test_count'] += 1
                
                # 检查正确性并更新指标
                is_correct = sentiment == true_sentiment
                if use_api and is_correct:
                    st.session_state.research_data['glm4_correct'] += 1
                elif not use_api and is_correct:
                    st.session_state.research_data['rule_correct'] += 1
                
                # 更新性能指标
                data = st.session_state.research_data
                model_key = 'GLM-4' if use_api else '规则方法'
                data['performance_metrics'][model_key]['accuracy'] = (
                    data['glm4_correct' if use_api else 'rule_correct'] / data['test_count']
                )
                data['performance_metrics'][model_key]['avg_time'] = (
                    data['performance_metrics'][model_key]['avg_time'] * (data['test_count']-1) + analysis_time
                ) / data['test_count']
                data['performance_metrics'][model_key]['total_tests'] = data['test_count']
                
                st.info(f"✅ 已记录研究数据，总测试次数: {data['test_count']}")
        ...
        if analyze_clicked:
            if st.session_state.current_case and st.session_state.current_case.strip():
                with st.spinner("分析案例中..."):
                    sentiment, confidence, status, analysis_time = analyze_sentiment_api(st.session_state.current_case)
...
with tab4:
    ...
    with col_about1:
        ...
        **研究创新点：**
        - 🔄 **双模式研究平台** - 结合实时演示与动态研究
        - 🤖 **智能案例生成** - AI实时生成多样化测试案例
        - 📊 **数据驱动洞察** - 基于实际测试数据生成研究结论
        - 💡 **渐进式学习** - 系统随测试积累而"成长"
...
# 页脚信息
st.markdown("---")
st.caption("🎯 基于Streamlit部署 | 📚 学年论文研究成果演示 | 🔬 动态研究平台 | 👨‍🎓 作者: wws")
...
with st.sidebar:
    ...
    **智能微博情感分析研究平台**
    ...
    data = st.session_state.research_data
    st.metric("总测试次数", data['test_count'])
    st.metric("研究开始", datetime.now().strftime("%H:%M"))
    
    st.subheader("🔍 快速测试")
    test_text = st.text_input("输入测试文本:", "这个功能很棒！")
    if st.button("快速分析", use_container_width=True):
        with st.spinner("分析中..."):
            s, c, t, _ = analyze_sentiment_local(test_text)
        st.write(f"结果: {s} ({c:.1%})")
